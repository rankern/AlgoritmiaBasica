...
class Problema{
	...
	public:
		...
		double costeEstimado(Pedido pedidos[], int numPedidos){
			...
			//	int j = this->pedidoActual + 1;
			//	while (replica[i] < this->maxCapacidad && j < numPedidos)
			//	{
			//		if(pedidos[j].estacionIni() <= i && i < pedidos[j].estacionFin()){
			//			int bill = pedidos[j].billetes();
			//			if(this->maxCapacidad - replica[i] < bill ){
			//				bill = this->maxCapacidad - replica[i];
			//			}
			//			coste += bill;
			//			replica[i] += bill;
			//		}
